...
namespace NFe.Danfe.Nativo.NFCe
{
    public class DanfeNativoNfce
    {
            ...
            if (!string.IsNullOrEmpty(salvarArquivoPdfEm))
            {
				foreach (var item in PrinterSettings.InstalledPrinters)
				{
					if (item.ToString().Contains("PDF")){
						printCupom.PrinterSettings.PrinterName = item.ToString();
					}
				}
        ...
        private void GerarNfCe(Graphics graphics)
        {
            ...
            _y = EscreverLinhaTitulo(g, razaoSocial.ToUpper(), 8, larguraLogo, x, _y, larguraLinha);

			string cnpj = Cnpj();

			_y = EscreverLinhaTitulo(g, cnpj, tamanhoFonteTitulo, larguraLogo, x, _y, larguraLinha);
		...
		private string Cnpj()
		{
			...
			string cnpj = string.Format($"CNPJ: {emitente.CNPJ}");
			return cnpj;
		}
		...
		private string RazaoSocial()
		{
			...
			string RazaoSocial = string.Format($"{nomeEmpresa}");
			return RazaoSocial;
		}
