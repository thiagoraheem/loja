...
namespace Loja.Modules
{
	public class NFCE
	{
		...
		public void ImprimirDanfe(string arquivoXml, string arquivoPDF = null)
		{
			...
			impr.Imprimir(salvarArquivoPdfEm: arquivoPDF);
		...
		protected virtual dest GetDestinatario(VersaoServico versao, ModeloDocumento modelo)
		{
			...
			if (_cliente != null)
			{
				if (_configuracoes.CfgServico.tpAmb.Equals(NFe.Classes.Informacoes.Identificacao.Tipos.TipoAmbiente.taHomologacao))
				{
		...
		protected virtual prod GetProduto(int i, tbl_SaidaItens item)
		{
			var p = new prod
			{
				...
				cEAN = "",
				xProd = _configuracoes.CfgServico.tpAmb.Equals(NFe.Classes.Informacoes.Identificacao.Tipos.TipoAmbiente.taHomologacao) && i == 1 ? "NOTA FISCAL EMITIDA EM AMBIENTE DE HOMOLOGACAO - SEM VALOR FISCAL" : item.tbl_Produtos.DesProduto,
