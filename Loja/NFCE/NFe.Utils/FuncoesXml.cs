...
namespace NFe.Utils
{
	public static class FuncoesXml
	{
		...
		/// <summary>
		///     Carrega o uma string com dados do arquivo XML.
		/// </summary>
		/// <param name="arquivo">Arquivo XML</param>
		/// <returns>Retorna uma string</returns>
		public static string ArquivoXmlParaString(string arquivo)
		{
			if (!File.Exists(arquivo))
			{
				throw new FileNotFoundException("Arquivo " + arquivo + " não encontrado!");
			...
			var stream =  System.IO.File.ReadAllText(arquivo);
			//new FileStream(arquivo, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
			try
			{
				return stream;
			}
